...
#ifndef __ATLWIN_H__
#error WtlEasySize.h requires atlwin.h to be included first
#endif
...
void __ES__RepositionControls(BOOL bInit, BOOL bRepaint);\
void __ES__CalcBottomRight(CWindow *pThis, BOOL bBottom, int &bottomright, int &topleft, UINT id, UINT br, int es_br, RECT &rect, int clientbottomright);
#define INIT_EASYSIZE __ES__RepositionControls(TRUE, FALSE); __ES__RepositionControls(FALSE, FALSE)
#define UPDATE_EASYSIZE(bRepaint) if(GetWindow(GW_CHILD)!=NULL) __ES__RepositionControls(FALSE, bRepaint)
...
void class::__ES__CalcBottomRight(CWindow *pThis, BOOL bBottom, int &bottomright, int &topleft, UINT id, UINT br, int es_br, RECT &rect, int clientbottomright) {\
...
void class::__ES__RepositionControls(BOOL bInit, BOOL bRepaint) { RECT rect,rect2,client; GetClientRect(&client);
...
if(o & ES_HCENTER) id##_es_l = (rect.right - rect.left)/2; else {\
if(l==ES_BORDER) id##_es_l = rect.left; else if(l==ES_KEEPSIZE) id##_es_l = (rect.right - rect.left); else {\
	GetDlgItem(l).GetWindowRect(&rect2); ScreenToClient(&rect2);\
	id##_es_l = rect.left-rect2.right;}}\
if(o & ES_VCENTER) id##_es_t = (rect.bottom - rect.top)/2; else {\
if(t==ES_BORDER) id##_es_t = rect.top; else if(t==ES_KEEPSIZE) id##_es_t = (rect.bottom - rect.top); else {\
	GetDlgItem(t).GetWindowRect(&rect2); ScreenToClient(&rect2);\
	id##_es_t = rect.top-rect2.bottom;}}\
if(o & ES_HCENTER) id##_es_r = (rect.right - rect.left); else { if(r==ES_BORDER) id##_es_r = client.right-rect.right; else if(r==ES_KEEPSIZE) id##_es_r = (rect.right - rect.left); else {\
	GetDlgItem(r).GetWindowRect(&rect2); ScreenToClient(&rect2);\
	id##_es_r = rect2.left-rect.right;}}\
if(o & ES_VCENTER) id##_es_b = (rect.bottom - rect.top); else  { if(b==ES_BORDER) id##_es_b = client.bottom-rect.bottom; else if(b==ES_KEEPSIZE) id##_es_b = (rect.bottom - rect.top); else {\
...
GetDlgItem(id).RedrawWindow(NULL, 0, RDW_FRAME|RDW_UPDATENOW|RDW_ERASE|RDW_INVALIDATE);}
#endif //__EASYSIZE_H_